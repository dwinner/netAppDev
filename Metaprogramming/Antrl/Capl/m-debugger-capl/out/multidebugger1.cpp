...
extern const char* g_IndexedNamestringArrray;
extern CAPL_HANDLE GetInternalCaplThread();
template<class T>T& CPPFunctionNotImplementedDummy(T i) {static T ret; ret = i; return ret; }

namespace capl
{

   extern HMODULE GetThisModuleHandle();

   extern CAPL_INT32 CAPL_CALL getAbsFilePath(const char *relPath, char *absPath, CAPL_INT32 absPathLen);
}; //namespace capl

#define FILE_SPECIFIC_FUNCTIONS
namespace capl
{
   //####################################
   //Dateiabhängige Hilfsfunktionen
   //####################################

enum IL_State { IL_Stopped, IL_Started, IL_Suspended };   static struct{const char acName[1];const char acHandle[1];DWORD u32BusActive;const BYTE u8Channel; IL_State ilState; bool boSimulationOn; bool boInitCalled; } s_asBusContext[1] = {"","",3,1, IL_Started, true, false}; // Buskontexbezeichnerliste.

   static unsigned short s_u16BusContext = 0; // Buskontext initial auf dem ersten Bus.

   static const char s_ecuName[1] = "";
   static const char s_fileName[5] = "main";

   static DWORD GetBusContext()
   {
      return (DWORD)s_u16BusContext + 0x10001;
   }

   static DWORD SetBusContext(DWORD context)
   {
      if(context == 0)
         return GetBusContext();
      unsigned short i = context - 1;
      if(i >= (sizeof(s_asBusContext)/sizeof(s_asBusContext[0])))
         return GetBusContext();
      s_u16BusContext = 0;
      for(; i < (sizeof(s_asBusContext)/sizeof(s_asBusContext[0])); ++i)
         if(strlen(s_asBusContext[i].acHandle) > 0)
         {
            s_u16BusContext = i;
            return GetBusContext();
         }
      for(i =(sizeof(s_asBusContext)/sizeof(s_asBusContext[0])); i > 0; --i)
         if(strlen(s_asBusContext[i].acHandle) > 0)
            s_u16BusContext = i;
      return GetBusContext();
   }

   static DWORD GetBusNameContext(const char* name)
   {
      unsigned short i = 0;
      for(i = 0; i < (sizeof(s_asBusContext)/sizeof(s_asBusContext[0])); ++i)
         if(strlen(s_asBusContext[i].acName) == strlen(name) && _strnicmp(s_asBusContext[i].acName, name, sizeof(s_asBusContext[i].acName)) == 0 && strlen(s_asBusContext[i].acHandle) > 0)
            return (DWORD)i + 0x10001;
      return 0;
   }

   static void CAPL_CALL DNMPutCmd(DWORD flag)
   {
      OSEKNM_DNMPutCmd(s_asBusContext[s_u16BusContext].acHandle, flag);
   }

   static void CAPL_CALL TalkNM(void)
   {
      OSEKNM_TalkNM(s_asBusContext[s_u16BusContext].acHandle);
   }

   static void CAPL_CALL SilentNM(void)
   {
      OSEKNM_SilentNM(s_asBusContext[s_u16BusContext].acHandle);
   }

   static void CAPL_CALL GotoMode_BusSleep(void)
   {
      OSEKNM_GotoMode_BusSleep(s_asBusContext[s_u16BusContext].acHandle);
   }

   static void CAPL_CALL GotoMode_Awake(void)
   {
      OSEKNM_GotoMode_Awake(s_asBusContext[s_u16BusContext].acHandle);
   }

   template<class TArray, class TOutput>
   inline TOutput& DebugArrayAccess(TArray& arr, TOutput& outDummy, const size_t& pos)
   {
      if((sizeof(arr)/sizeof(arr[0])) <= pos)
         RaiseException(EXCEPTION_ARRAY_BOUNDS_EXCEEDED, 0, 0, NULL);
      return arr[pos];
   }

   template<>
   inline TimerArrayEvent::ChildEvent& DebugArrayAccess(TimerArrayEvent& arr, TimerArrayEvent::ChildEvent& outDummy, const size_t& pos)
   {
      return arr[pos];
   }
   ...
   {
      write( "d:\\work-files\\multidebugger\\main.can");
   }
...
//####################################
//PluginWrapperApiVerbatimInclude DONE
//####################################
//####################################
//PluginCanFileSpecificApiVerbatimInclude
//####################################
// Damit dies Funktioniert, muss der Code vom Namespace "capl" umgeben werden.
   ...
   ////////////////////////////////////////////////////////
   //
   //        Zugriff auf Bus- und Ecu-Referenzen
   //
   ////////////////////////////////////////////////////////

    static LPCSTR GetCurBusContext()
    {
        return s_asBusContext[s_u16BusContext].acHandle;
    }

    static LPCSTR GetEcuNameInternal()
    {
        return s_ecuName;
    }

    static LPCSTR GetEcuRefInternal()
    {
        static char ecuRef[300];
        strcpy(ecuRef, GetCurBusContext());
        strcat(ecuRef, "/");
        strcat(ecuRef, GetEcuNameInternal());
        return ecuRef;
    }

    static LPCSTR GetFileNameInternal()
    {
        return s_fileName;
    }

    // Funktion memcpy.
    static void memcpy(void* pDest, const void* pSource, DWORD length)
    {
        ::memcpy(pDest, pSource, length);
    }

    template<class DEST, class SOURCE>
    static void memcpy_off(DEST& dest, DWORD destOffset, SOURCE& src, DWORD sourceOffset, DWORD length)
    {
        ::memcpy((BYTE*)&dest + destOffset, (BYTE*)&src + sourceOffset, length);
    }
    ...
    // Callbackfunktionen für Ethernet
    static OnSendFunction* s_OnUdpSendToFunction = NULL;
    static OnSendFunction* s_OnTcpSendFunction = NULL;
    static OnUdpReceiveFromFunctionIPv4* s_OnUdpReceiveFromFunctionIPv4 = NULL;
    static OnUdpReceiveFromFunctionIPv6* s_OnUdpReceiveFromFunctionIPv6 = NULL;
    static OnTcpReceiveFunctionIPv4* s_OnTcpReceiveFunctionIPv4 = NULL;
    static OnTcpReceiveFunctionIPv6* s_OnTcpReceiveFunctionIPv6 = NULL;
    struct OnTcpSendFunctionSetter { OnTcpSendFunctionSetter(OnSendFunction* pFunc) { s_OnTcpSendFunction = pFunc; } };
    struct OnUdpSendToFunctionSetter { OnUdpSendToFunctionSetter(OnSendFunction* pFunc) { s_OnUdpSendToFunction = pFunc; } };
    struct OnUdpReceiveFromFunctionSetterIPv4 { OnUdpReceiveFromFunctionSetterIPv4(OnUdpReceiveFromFunctionIPv4* pFunc) { s_OnUdpReceiveFromFunctionIPv4 = pFunc; } };
    struct OnUdpReceiveFromFunctionSetterIPv6 { OnUdpReceiveFromFunctionSetterIPv6(OnUdpReceiveFromFunctionIPv6* pFunc) { s_OnUdpReceiveFromFunctionIPv6 = pFunc; } };
    struct OnTcpReceiveFunctionSetterIPv4 { OnTcpReceiveFunctionSetterIPv4(OnTcpReceiveFunctionIPv4* pFunc) { s_OnTcpReceiveFunctionIPv4 = pFunc; } };
    struct OnTcpReceiveFunctionSetterIPv6 { OnTcpReceiveFunctionSetterIPv6(OnTcpReceiveFunctionIPv6* pFunc) { s_OnTcpReceiveFunctionIPv6 = pFunc; } };

    static CAPL_INT32 UdpSendToIP4(DWORD socket, DWORD address, DWORD port, void* buffer, DWORD size)
    {
        return UdpSendToIP4Internal(s_OnUdpSendToFunction, socket, address, port, buffer, size);
    }

    static CAPL_INT32 UdpSendToIP6(DWORD socket, byte ipv6Address[], DWORD port, void* buffer, DWORD size)
    {
        return UdpSendToIP6Internal(s_OnUdpSendToFunction, socket, ipv6Address, port, buffer, size);
    }

    static CAPL_INT32 TcpSend(DWORD socket, void* buffer, DWORD size)
    {
        return TcpSendInternal(s_OnTcpSendFunction, socket, buffer, size);
    }

    static CAPL_INT32 UdpReceiveFrom(DWORD socket, void* buffer, DWORD size)
    {
        return UdpReceiveFromInternal(s_OnUdpReceiveFromFunctionIPv4, s_OnUdpReceiveFromFunctionIPv6, socket, buffer, size);
    }

    static CAPL_INT32 TcpReceive(DWORD socket, void* buffer, DWORD size)
    {
        return TcpReceiveInternal(s_OnTcpReceiveFunctionIPv4, s_OnTcpReceiveFunctionIPv6, socket, buffer, size);
    }

    static CAPL_INT32 EthReceivePacket(const char* sCallback)
    {
        HMODULE hModule = GetThisModuleHandle();
        OnEthernetFunction* pCallback = (OnEthernetFunction*)GetProcAddress(hModule, sCallback);
        return EthReceivePacketInternal(pCallback);
    }

    static CAPL_INT32 EthReceiveRawPacket(long flags, byte srcMacAddress[], byte dstMacAddress[6], long ethernetType, const char* sCallback)
    {
        HMODULE hModule = GetThisModuleHandle();
        OnEthernetFunction* pCallback = (OnEthernetFunction*)GetProcAddress(hModule, sCallback);
        return EthReceiveRawPacketInternal(pCallback, flags, srcMacAddress, dstMacAddress, ethernetType);;
    }

    static DWORD TcpOpenIP4(DWORD address, DWORD port)
    {
        return TcpOpenIP4Internal(GetFileNameInternal(), address, port);
    }

    static DWORD TcpOpenIP6(byte ipv6Address[], DWORD port)
    {
        return TcpOpenIP6Internal(GetFileNameInternal(), ipv6Address, port);
    }


    static DWORD UdpOpenIP4(DWORD address, DWORD port)
    {
        return UdpOpenIP4Internal(GetFileNameInternal(), address, port);
    }

    static DWORD UdpOpenIP6(byte ipv6Address[], DWORD port)
    {
        return UdpOpenIP6Internal(GetFileNameInternal(), ipv6Address, port);
    }

    static DWORD IpGetAdapterCount()
    {
        return IpGetAdapterCountInternal(GetFileNameInternal());
    }

    static CAPL_INT32 IpGetAdapterAddressCount(DWORD ifIndex, DWORD addressFamily)
    {
        return IpGetAdapterAddressCountInternal(GetFileNameInternal(), ifIndex, addressFamily);
    }

    static CAPL_INT32 IpGetAdapterAddressIPv4(DWORD ifIndex, DWORD address[], DWORD count)
    {
        return IpGetAdapterAddressIPv4Internal(GetFileNameInternal(), ifIndex, address, count);
    }

    static CAPL_INT32 IpGetAdapterAddressIPv6(DWORD ifIndex, byte ipv6Addresses[][16], DWORD count)
    {
        return IpGetAdapterAddressIPv6Internal(GetFileNameInternal(), ifIndex, ipv6Addresses, count);
    }

    static CAPL_INT32 IpAddAdapterAddressIPv4(DWORD ifIndex, DWORD address, DWORD mask)
    {
        return IpAddAdapterAddressIPv4Internal(GetFileNameInternal(), ifIndex, address, mask);
    }

    static CAPL_INT32 IpAddAdapterAddressIPv6(DWORD ifIndex, byte ipv6Address[], DWORD prefix)
    {
        return IpAddAdapterAddressIPv6Internal(GetFileNameInternal(), ifIndex, ipv6Address, prefix);
    }

    static CAPL_INT32 IpRemoveAdapterAddressIPv4(DWORD ifIndex, DWORD address, DWORD mask)
    {
        return IpRemoveAdapterAddressIPv4Internal(GetFileNameInternal(), ifIndex, address, mask);
    }

    static CAPL_INT32 IpRemoveAdapterAddressIPv6(DWORD ifIndex, byte ipv6Address[], DWORD prefix)
    {
        return IpRemoveAdapterAddressIPv6Internal(GetFileNameInternal(), ifIndex, ipv6Address, prefix);
    }

    static CAPL_INT32 IpGetAdapterDescription(DWORD ifIndex, char name[], DWORD count)
    {
        return IpGetAdapterDescriptionInternal(GetFileNameInternal(), ifIndex, name, count);
    }

    static CAPL_INT32 IpJoinMulticastGroupIP6(DWORD socket, DWORD ifIndex, byte ipv6Address[])
    {
        return IpJoinMulticastGroupIP6Internal(GetFileNameInternal(), socket, ifIndex, ipv6Address);
    }

    static CAPL_INT32 IpJoinMulticastGroupIP4(DWORD socket, DWORD ifIndex, DWORD address)
    {
        return IpJoinMulticastGroupIP4Internal(GetFileNameInternal(), socket, ifIndex, address);
    }

    static CAPL_INT32 IpSetMulticastInterface(DWORD socket, DWORD ifIndex)
    {
        return IpSetMulticastInterfaceInternal(GetFileNameInternal(), socket, ifIndex);
    }

    ////////////////////////////////////////////////////////
    //
    //         Netwerkmanagement Funktionen
    //
    ////////////////////////////////////////////////////////

    // Callbackfunktionen für NM.
    static NMHighFunction* s_NMHighSleepFunction = NULL;
    static NMHighFunction* s_NMHighWakeupFunction = NULL;
    static NMHighFunction* s_NMHighPrepareSleepFunction = NULL;
    static NMHighFunction* s_NMHighStartIndFunction = NULL;
    static NMHighRemoteFunction* s_NMHighRemoteFunction = NULL;
    static NMHighStateChangeFunction* s_NMHighStateChangeFunction = NULL;

    // Wird vom Printer ausgegeben, wenn ein Callback verwendet wird.
    // Überschreibt den globalen Zeiger (siehe oben) auf den Callback.
    struct NMHighSleepFunctionSetter { NMHighSleepFunctionSetter(NMHighFunction* pFunc) { s_NMHighSleepFunction = pFunc; } };
    struct NMHighWakeupFunctionSetter { NMHighWakeupFunctionSetter(NMHighFunction* pFunc) { s_NMHighWakeupFunction = pFunc; } };
    struct NMHighPrepareSleepFunctionSetter { NMHighPrepareSleepFunctionSetter(NMHighFunction* pFunc) { s_NMHighPrepareSleepFunction = pFunc; } };
    struct NMHighRemoteFunctionSetter { NMHighRemoteFunctionSetter(NMHighRemoteFunction* pFunc) { s_NMHighRemoteFunction = pFunc; } };
    struct NMHighStateChangeSetter { NMHighStateChangeSetter(NMHighStateChangeFunction* pFunc) { s_NMHighStateChangeFunction = pFunc; } };
    struct NMHighStartIndFunctionSetter { NMHighStartIndFunctionSetter(NMHighFunction* pFunc) { s_NMHighStartIndFunction = pFunc; } };

    static void NMH_SetApplicationActivity(CAPL_INT32 flag)
    {
        // not implemented
    }

    static void Nm_RequestBusCom(void)
    {
        NMH_RequestBusComInternal(GetCurBusContext(), GetEcuNameInternal());
    }

    static void Nm_ReleaseBusCom(void)
    {
        NMH_ReleaseBusComInternal(GetCurBusContext(), GetEcuNameInternal());
    }

    static void NMH_ReqNetOff(void)
    {
        Nm_ReleaseBusCom();
    }

    static void NMH_ReqNetOn(void)
    {
        Nm_RequestBusCom();
    }

    static void NMH_SetVerbosity(DWORD level)
    {
        // not implemented.
    }
    ...
    static void Nm_SetUserData(DWORD data)
    {
        ...
        buffer[6] = pData[1];
        buffer[7] = pData[0];
        NMH_SetUserDataInternal(GetCurBusContext(), GetEcuNameInternal(), buffer, sizeof(buffer));
    }

    //static void Nm_SetUserDataByteArray(const BYTE* pData, DWORD length)
    static void Nm_SetUserData(const BYTE* pData, DWORD length)
    {
        NMH_SetUserDataInternal(GetCurBusContext(), GetEcuNameInternal(), pData, length);
    }

    static CAPL_INT32 Nm_PassiveStartUp()
    {
        return NMH_PassiveStartUp(GetCurBusContext(), GetEcuNameInternal());
    }

    static CAPL_INT32 Nm_SetApplicationActivity(CAPL_INT32 isActive)
    {
        // not implemented.
        return 0;
    }

    static CAPL_INT32 Nm_NetworkRequest()
    {
        Nm_RequestBusCom();
        return 0;
    }

    static CAPL_INT32 Nm_NetworkRelease()
    {
        Nm_ReleaseBusCom();
        return 0;
    }
    static CAPL_INT32 Nm_EnableCommunication()
    {
        // not implemented
        return 0;
    }
    static CAPL_INT32 Nm_DisableCommunication()
    {
        // not implemented
        return 0;
    }
    ...
    namespace {
        class NMHActivateEcu
        {
        public:
            NMHActivateEcu()
            {
                for (unsigned i = 0; i < (sizeof(s_asBusContext) / sizeof(s_asBusContext[0])); ++i)
                    if (strlen(s_asBusContext[i].acHandle) > 0)
                        capl::NMH_Init(s_asBusContext[i].acHandle, GetEcuNameInternal(), s_NMHighSleepFunction, s_NMHighWakeupFunction, s_NMHighPrepareSleepFunction, s_NMHighRemoteFunction, s_NMHighStateChangeFunction, s_NMHighStartIndFunction);
            }
            ~NMHActivateEcu()
            {
                for (unsigned i = 0; i < (sizeof(s_asBusContext) / sizeof(s_asBusContext[0])); ++i)
                    if (strlen(s_asBusContext[i].acHandle) > 0)
                        capl::NMH_DeInit(s_asBusContext[i].acHandle, GetEcuNameInternal());
            }
        };
    }

    static CAPL_INT32 NM_GetStationAddr()
    {
        return NMH_GetStationAddrInternal(GetCurBusContext(), GetEcuNameInternal());
    }

    ////////////////////////////////////////////////////////
    //
    //         Interaction-Layer Funktionen
    //
    ////////////////////////////////////////////////////////

    static CAPL_INT32 ILControlInit()
    {
        if (s_asBusContext[s_u16BusContext].boInitCalled)
            return -1;

        s_asBusContext[s_u16BusContext].boInitCalled = true;
        s_asBusContext[s_u16BusContext].ilState = IL_Stopped;
        return ILControlInitInternal(GetEcuRefInternal());
    }

    static CAPL_INT32 ILControlStart()
    {
        if (!s_asBusContext[s_u16BusContext].boSimulationOn)
            return -1;
        if (s_asBusContext[s_u16BusContext].ilState != IL_Stopped)
            return -1;
        s_asBusContext[s_u16BusContext].ilState = IL_Started;
        return ILControlStartInternal(GetEcuRefInternal());
    }

    static CAPL_INT32 ILControlStop()
    {
        if (!s_asBusContext[s_u16BusContext].boSimulationOn)
            return -1;
        if (s_asBusContext[s_u16BusContext].ilState != IL_Started)
            return -1;
        s_asBusContext[s_u16BusContext].ilState = IL_Stopped;
        return ILControlStopInternal(GetEcuRefInternal());
    }

    static CAPL_INT32 ILControlWait()
    {
        if (!s_asBusContext[s_u16BusContext].boSimulationOn)
            return -1;
        if (s_asBusContext[s_u16BusContext].ilState != IL_Started)
            return -1;
        s_asBusContext[s_u16BusContext].ilState = IL_Suspended;
        return ILControlWaitInternal(GetEcuRefInternal());
    }

    static CAPL_INT32 ILControlResume()
    {
        if (!s_asBusContext[s_u16BusContext].boSimulationOn)
            return -1;
        if (s_asBusContext[s_u16BusContext].ilState != IL_Suspended)
            return -1;
        s_asBusContext[s_u16BusContext].ilState = IL_Started;
        return ILControlStartInternal(GetEcuRefInternal());
    }

    static CAPL_UINT32 ILControlResume(CAPL_UINT32)
    {
        return (CAPL_UINT32)ILControlResume();
    }

    static CAPL_UINT32 ILControlWait(CAPL_UINT32)
    {
        return (CAPL_UINT32)ILControlWait();
    }
    
    static CAPL_INT32 ILActivateClamp15()
    {
        if (!s_asBusContext[s_u16BusContext].boSimulationOn)
            return -1;
        return ILActivateClamp15Internal(GetEcuRefInternal());
    }

    static CAPL_INT32 ILDeactivateClamp15()
    {
        if (!s_asBusContext[s_u16BusContext].boSimulationOn)
            return -1;
        return ILDeactivateClamp15Internal(GetEcuRefInternal());
    }

    static CAPL_INT32 ILControlSimulationOn()
    {
        if (s_asBusContext[s_u16BusContext].boSimulationOn)
            return -1;
        s_asBusContext[s_u16BusContext].boSimulationOn = true;
        return ILControlStart();
    }

    static CAPL_INT32 ILControlSimulationOff()
    {
        if (!s_asBusContext[s_u16BusContext].boSimulationOn)
            return -1;
        ILControlStop();
        s_asBusContext[s_u16BusContext].boSimulationOn = false;
        return 0;
    }

    static CAPL_INT32 ILNodeControlSimulationOn(const char* sEcu)
    {
        return ILControlStartInternal(sEcu);
    }

    static CAPL_INT32 ILNodeControlSimulationOff(const char* sEcu)
    {
        return ILControlStopInternal(sEcu);
    }

    static CAPL_INT32 ILSetMsgEvent(LPCTSTR sMsgRef)
    {
        if (!s_asBusContext[s_u16BusContext].boSimulationOn)
            return -1;
        ...
        return ILSetMsgEventInternal(sMsgRef);
    }

    static CAPL_INT32 NM_CtrlSimulationOff()
    {
        return ILControlSimulationOff();
    }

    static CAPL_INT32 NM_CtrlSimulationOn()
    {
        return ILControlSimulationOn();
    }

    static CAPL_INT32 NM_ActivateClamp15()
    {
        return ILActivateClamp15();
    }

    static CAPL_INT32 NM_DeactivateClamp15()
    {
        return ILDeactivateClamp15();
    }
    ...
    static DWORD canOnline(DWORD flags)
    {
        DWORD canFlags = s_asBusContext[s_u16BusContext].u32BusActive;
        s_asBusContext[s_u16BusContext].u32BusActive |= flags;

        const bool boNodeActiveNew = flags & 2;
        const bool boNodeActiveOld = canFlags & 2;
        if (boNodeActiveNew != boNodeActiveOld)
            capl::NofityEcuStateChange(GetEcuRefInternal(), true);

        return canFlags;
    }

    static void canOnline() { canOnline(1); }

    static DWORD canOffline(DWORD flags)
    {
        DWORD canFlags = s_asBusContext[s_u16BusContext].u32BusActive;
        s_asBusContext[s_u16BusContext].u32BusActive &= ~flags;

        const bool boNodeActiveNew = s_asBusContext[s_u16BusContext].u32BusActive & 2;
        const bool boNodeActiveOld = canFlags & 2;
        if (boNodeActiveNew != boNodeActiveOld)
            capl::NofityEcuStateChange(GetEcuRefInternal(), false);

        return canFlags;
    }

    static void canOffline() { canOffline(1); }
    ...
    // Callback-Funktionen die vom Plugin überschrieben werden können.
    static RS232OnSendFunc* s_pRS232OnSendFunc = NULL;
    static R232ByteCallbackFunc* s_pByteCallbackFunc = NULL;
    static R232ByteCallbackFuncInt* s_pByteCallbackFuncInt = NULL;
    static RS232OnErrorFunc* s_pOnErrorFunc = NULL;
    static R232ReceivedFunc* s_pReceivedFunc = NULL;

    // Wird vom Printer ausgegeben, wenn ein Callback verwendet wird.
    // Überschreibt den globalen Zeiger (siehe oben) auf den Callback.
    struct RS232_ByteCallback { RS232_ByteCallback(R232ByteCallbackFunc* pFunc) { s_pByteCallbackFunc = pFunc; } RS232_ByteCallback(R232ByteCallbackFuncInt* pFunc) { s_pByteCallbackFuncInt = pFunc; } };
    struct RS232_OnSendCallback { RS232_OnSendCallback(RS232OnSendFunc* pFunc) { s_pRS232OnSendFunc = pFunc; } };
    struct RS232_OnErrorCallback { RS232_OnErrorCallback(RS232OnErrorFunc* pFunc) { s_pOnErrorFunc = pFunc; } };
    struct RS232_OnReceiveCallback { RS232_OnReceiveCallback(R232ReceivedFunc* pFunc) { s_pReceivedFunc = pFunc; } };

    static DWORD RS232Open(DWORD port)
    {
        if (s_pByteCallbackFunc != NULL)
            return RS232OpenInternal(port, s_pRS232OnSendFunc, s_pByteCallbackFunc, s_pOnErrorFunc);
        else
            return RS232OpenInternalInt(port, s_pRS232OnSendFunc, s_pByteCallbackFuncInt, s_pOnErrorFunc);
    }

    static DWORD RS232SetCommState(DWORD port, DWORD baudrate, DWORD numberOfDataBits, DWORD numberOfStopBits, DWORD parity)
    {
        if (RS232Open(port) == 0)
            return 0;
        return RS232Configure(port, baudrate, numberOfDataBits, numberOfStopBits, parity);
    }

    static DWORD RS232SetHandshake(DWORD port, DWORD handshake, DWORD XonLimit, DWORD XoffLimit, DWORD XonChar, DWORD XoffChar, DWORD timeout = -1)
    {
        return RS232SetHandshakeInternal(port, handshake, XonLimit, XoffLimit, XonChar, XoffChar, timeout);
    }

    static DWORD RS232Receive(DWORD port, BYTE* pBuffer, DWORD size)
    {
        return RS232ReceiveInternal(port, pBuffer, size, s_pReceivedFunc);
    }

    ////////////////////////////////////////////////////////
    //
    //                AIDA Initialisierung
    //
    ////////////////////////////////////////////////////////

    static CAPL_HANDLE s_hDiagStack = NULL;

    static void AIDA_Init(const char* sConfig)
    {
        if (s_hDiagStack == NULL)
            s_hDiagStack = AIDA_InitInternal(GetCurBusContext(), sConfig);
    }

    namespace
    {
        template<class Notifier>
        class ReceptionIndSetter : public PreStartEvent
        {
        public:
            ReceptionIndSetter(const char* sConfig)
                : m_sConfig(sConfig)
                , m_hDataEvent(NULL)
            {
            }
            ...
            {
                if (m_hDataEvent)
                    Delete_ReceptionInd_Event(m_hDataEvent);
            }

        private:

            CAPL_METHOD(void, OnEvent)(IPreStart&)
            {
                AIDA_Init(m_sConfig.c_str());
                m_hDataEvent = Create_ReceptionInd_Event(&Notifier::Notify, s_hDiagStack);
            }

            std::string m_sConfig;
            CAPL_HANDLE m_hDataEvent;
        };

        template<class Notifier>
        class SendConSetter : public PreStartEvent
        {
        public:
            SendConSetter(const char* sConfig)
                : m_sConfig(sConfig)
                , m_hDataEvent(NULL)
            {
            }
            ...
            {
                if (m_hDataEvent)
                    Delete_SendCon_Event(m_hDataEvent);
            }
        private:
            CAPL_METHOD(void, OnEvent)(IPreStart&)
            {
                AIDA_Init(m_sConfig.c_str());
                m_hDataEvent = Create_SendCon_Event(&Notifier::Notify, s_hDiagStack);
            }
        ...
        Transmission_Function* s_pSendConf_Func = NULL;
        Transmission_Function* s_pReceptionInd_Func = NULL;

        template<class Notifier>
        class CanTpReceptionIndSetter
        {
        public:
            CanTpReceptionIndSetter()
            {
                s_pReceptionInd_Func = &Notifier::Notify;
            }
        };

        template<class Notifier>
        class CanTpSendConSetter
        {
        public:
            CanTpSendConSetter()
            {
                s_pSendConf_Func = &Notifier::Notify;
            }
        };
    }
    ...
    typedef void (TP20_SetupCon)(void);
    static TP20_SetupCon* s_pTP20SetupCon = NULL;

    namespace {
        class TP20_SetupConSetter
        {
        public:
            TP20_SetupConSetter(TP20_SetupCon* pFunc)
            {
                s_pTP20SetupCon = pFunc;
            }
        };
    }

    // Request by the application to set up the channel.
    static void VWTP2_DataReq(const BYTE* pData, CAPL_INT32 txCount)
    {
        AIDA_DataReqInternal(s_hDiagStack, pData, txCount);
    }

    // The received data can be retrieved from the Transport Layer by using this function within the func-tions VWTP2_DataInd and VWTP2_FirstFrameInd.
    static void VWTP2_GetRxData(BYTE* rxData, CAPL_INT32 rxSize)
    {
        AIDA_GetRxDataInternal(s_hDiagStack, rxData, rxSize);
    }

    // Sends a CS to the communication partner with the latest values set for block size and timers.
    static void VWTP2_SetupReq()
    {
        if (s_pTP20SetupCon)
            s_pTP20SetupCon();
    }
    ...
    static void OSEKTL_Init()
    {
        AIDA_Init("BSKD\\CAN_USDT_BDIAG.aida-cfg");
    }

    static void OSEKTL_DataReq(const BYTE* pData, CAPL_INT32 iBufferSize)
    {
        OSEKTL_Init();
        AIDA_DataReqInternal(s_hDiagStack, pData, iBufferSize);
    }

    static void OSEKTL_GetRxData(BYTE* pData, CAPL_INT32 lBufferSize)
    {
        OSEKTL_Init();
        AIDA_GetRxDataInternal(s_hDiagStack, pData, lBufferSize);
    }

    static void OSEKTL_SetTxId(DWORD txId)
    {
        OSEKTL_Init();
        OSEKTL_SetTxIdInternal(s_hDiagStack, txId);
    }

    static void OSEKTL_SetRxId(DWORD rxId)
    {
        OSEKTL_Init();
        OSEKTL_SetRxIdInternal(s_hDiagStack, rxId);
    }

    static CAPL_INT32 OSEKTL_GetTxId()
    {
        OSEKTL_Init();
        return OSEKTL_GetTxIdInternal(s_hDiagStack);
    }

    static CAPL_INT32 OSEKTL_GetRxId()
    {
        OSEKTL_Init();
        return OSEKTL_GetRxIdInternal(s_hDiagStack);
    }

    static CAPL_INT32 s_OSEKTL_EcuNumber = 0;
    static void OSEKTL_SetEcuNumber(CAPL_INT32 val)
    {
        s_OSEKTL_EcuNumber = val;
    }


    static CAPL_INT32 OSEKTL_GetEcuNumber()
    {
        return s_OSEKTL_EcuNumber;
    }

    static CAPL_INT32 s_OSEKTL_TgtAdr = 0;
    static void OSEKTL_SetTgtAdr(CAPL_INT32 val)
    {
        s_OSEKTL_TgtAdr = val;
    }

    static CAPL_INT32 OSEKTL_GetTgtAdr()
    {
        return s_OSEKTL_TgtAdr;
    }

    static CAPL_INT32 s_OSEKTL_TpBaseAdr = 0;
    static void OSEKTL_SetTpBaseAdr(CAPL_INT32 val)
    {
        s_OSEKTL_TpBaseAdr = val;
    }

    static CAPL_INT32 OSEKTL_GetTpBaseAdr()
    {
        return s_OSEKTL_TpBaseAdr;
    }

    static CAPL_INT32 CanTpCreateConnection(CAPL_INT32 mode)
    {
        CAPL_HANDLE hStack = CanTpCreateConnectionInternal(GetCurBusContext(), mode, s_pSendConf_Func, s_pReceptionInd_Func);
        return (CAPL_INT32)hStack;
    }

    static CAPL_INT32 s_OSEKTL_ConnectionCount = 0;
    static CAPL_INT32 CanTpCreateConnectionCounted(CAPL_INT32 mode)
    {
        ++s_OSEKTL_ConnectionCount;
        if (s_OSEKTL_ConnectionCount > 100)
        {
            static bool called = false;
            if (!called)
            {
                called = true;
                ShowToManyConnectionsMessageBox(GetEcuNameInternal(), GetFileNameInternal());
            }
        }
        return CanTpCreateConnection(mode);
    }

    static CAPL_INT32 CanTpCloseConnectionCounted(CAPL_INT32 hStack)
    {
        if(s_OSEKTL_ConnectionCount > 0)
            --s_OSEKTL_ConnectionCount;
        return CanTpCloseConnection(hStack);
    }

    static CAPL_INT32 EthOutputPacket(CAPL_INT32 packet)
    {
        return EthOutputPacketInternal(GetCurBusContext(), packet);
    }

    static void CAPL_CALL output(IMessage& msg)
    {
        DWORD u32BusActive = s_asBusContext[s_u16BusContext].u32BusActive;
        for (CAPL_UINT32 i = 0; i < (sizeof(s_asBusContext) / sizeof(s_asBusContext[0])); ++i)
            if (msg.CAN() == s_asBusContext[i].u8Channel)
            {
                if (strlen(s_asBusContext[i].acHandle) > 0)
                    u32BusActive = s_asBusContext[i].u32BusActive;
                break;
            }
    ...
    typedef std::pair<IEnvVar*, IEnvVarEvent*> EnvVarPair;
    typedef std::vector<EnvVarPair> EnvVarEventContainer;
    static EnvVarEventContainer s_EnvVarEventContainer;

    namespace {
        class EnvVarEvent : public IEnvVarEvent
        {
        public:
            EnvVarEvent(const char* sId)
            {
                m_hEvent = CreateEnvVarEvent(this, sId);
            }

            ~EnvVarEvent()
            {
                DeleteEnvVarEvent(m_hEvent);
            }

            CAPL_METHOD(void, SetEnvVar)(IEnvVar& envvar)
            {
                s_EnvVarEventContainer.push_back(EnvVarPair(&envvar, this));
            }
        };
        class EnvVarUpdateEvent : public IEnvVarEvent
        {
        public:
            EnvVarUpdateEvent(const char* sId)
            {
                m_hEvent = CreateEnvVarUpdateEvent(this, sId);
            }

            ~EnvVarUpdateEvent()
            {
                DeleteEnvVarUpdateEvent(m_hEvent);
            }
    ...
    // callAllOnEnvVar
    static void callAllOnEnvVar()
    {
        EnvVarEventContainer::iterator i(s_EnvVarEventContainer.begin()), end(s_EnvVarEventContainer.end());
        for (; i != end; ++i)
        {
            i->second->OnEvent(*i->first);
        }
    }


    // Zeitmessungsklasse wird vom CAPL-Converter automatisch ausgegeben,
    // wenn das "CAPL Messung" Plugin hinzugefügt wurde.
    void StartMessung(DWORD handle);
    void StopMessung(DWORD handle);
    class MessureWrapper
    {
    private:
        DWORD m_handle;
    public:
        MessureWrapper(DWORD handle) : m_handle(handle)
        {
            StartMessung(m_handle);
        }
        ~MessureWrapper()
        {
            StopMessung(m_handle);
        }
    };

    // DiagRequest/Response
    static char sStackName[STRLEN_FOR_SIGNAL_NAME];

    static CAPL_INT32 diagSetTarget(char stackName[])
    {
        strcpy(sStackName, stackName);
        diagTestStack(sStackName, stackName);
        return 0;
    }

    static CAPL_INT32 diagStartTesterPresent()
    {
        return diagStartTesterPresentObj(sStackName);
    }

    static CAPL_INT32 diagSetTimeoutHandler(const char* sCallback)
    {
        HMODULE hModule = GetThisModuleHandle();
        OnDiagTiemoutFunction* pCallback = (OnDiagTiemoutFunction*)GetProcAddress(hModule, sCallback);
        return diagSetTimeoutHandlerInternal(pCallback, sStackName);
    }

    static CAPL_INT32 diagSetTimeoutHandlerObj(capl::IDiagRequest& req, const char* sCallback)
    {
        HMODULE hModule = GetThisModuleHandle();
        OnDiagTiemoutFunction* pCallback = (OnDiagTiemoutFunction*)GetProcAddress(hModule, sCallback);
        return diagSetTimeoutHandlerObjInternal(pCallback, req);
    }

    static char* GetStackName()
    {
        return sStackName;
    }
    ...
    // Interface für Botschaften.
    class DiagBaseWrapper
    {
    protected:
        IDiagBase* m_pDiagBase;
    public:
        explicit DiagBaseWrapper(IDiagBase* pDiagBase)
            : m_pDiagBase(pDiagBase)
        {
        }

        virtual ~DiagBaseWrapper()
        {
        ...
        CAPL_METHOD_IMPL(CAPL_INT32, SetParameter)(char parameterName[], double newValue)
        {
            return m_pDiagBase->SetParameter(parameterName, newValue);
        }

        CAPL_METHOD_IMPL(CAPL_INT32, SetParameter)(CAPL_INT32 mode, char parameterName[], double newValue)
        {
            return m_pDiagBase->SetParameter(mode, parameterName, newValue);
        }

        CAPL_METHOD_IMPL(CAPL_INT32, SetParameter)(char parameterName[], char newValue[])
        {
            return m_pDiagBase->SetParameter(parameterName, newValue);
        }

        CAPL_METHOD_IMPL(CAPL_INT32, GetParameter)(char parameterName[], double output[1]) const
        {
            return m_pDiagBase->GetParameter(parameterName, output);
        }

        CAPL_METHOD_IMPL(double, GetParameter)(char parameterName[]) const
        {
            return m_pDiagBase->GetParameter(parameterName);
        }

        CAPL_METHOD_IMPL(CAPL_INT32, GetParameter)(CAPL_INT32 mode, char parameterName[], double output[1]) const
        {
            return m_pDiagBase->GetParameter(mode, parameterName, output);
        }

        CAPL_METHOD_IMPL(double, GetParameter)(CAPL_INT32 mode, char parameterName[]) const
        {
            return m_pDiagBase->GetParameter(mode, parameterName);
        }

        CAPL_METHOD_IMPL(CAPL_INT32, GetParameter)(char parameterName[], char buffer[], CAPL_UINT32 buffersize) const
        {
            return m_pDiagBase->GetParameter(parameterName, buffer, buffersize);
        }

        CAPL_METHOD_IMPL(CAPL_INT32, SetPrimitiveData)(CAPL_UINT8 buffer[], CAPL_UINT32 buffersize)
        {
            return m_pDiagBase->SetPrimitiveData(buffer, buffersize);
        }

        CAPL_METHOD_IMPL(CAPL_INT32, GetPrimitiveData)(CAPL_UINT8 buffer[], CAPL_UINT32 buffersize) const
        {
            return m_pDiagBase->GetPrimitiveData(buffer, buffersize);
        }

        CAPL_METHOD_IMPL(CAPL_INT32, Resize)(CAPL_UINT32 byteCount)
        {
            return m_pDiagBase->Resize(byteCount);
        }

        CAPL_METHOD_IMPL(CAPL_INT32, GetPrimitiveSize)() const
        {
            return m_pDiagBase->GetPrimitiveSize();
        }

        IDiagBase& operator=(IDiagBase& m)
        {
            m_pDiagBase->CopyData(m);
            return *m_pDiagBase;
        }
        DiagBaseWrapper& operator=(DiagBaseWrapper& m)
        {
            m_pDiagBase->CopyData(*m.m_pDiagBase);
            return *this;
        }

    };
    class DiagResponseWrapper : public DiagBaseWrapper
    {
        IDiagResponse* m_pDiagResp;
        /* Definiert ob m_pMsg durch die Erzeugung dieses Wrapper-Objektes
        * ebenfalls erzeugt wurde und somit auch von diesem wieder freigegeben
        * werden muss.
        */
        bool m_boDeleteMessage;
    public:
        explicit DiagResponseWrapper(const char* sHandle = "")
            : DiagBaseWrapper(NULL)
            , m_boDeleteMessage(true)
        {
            m_pDiagBase = m_pDiagResp = CreateDiagResponse(sHandle, GetStackName());
        }

        explicit DiagResponseWrapper(IDiagResponse& msg)
            : DiagBaseWrapper(&msg)
            , m_boDeleteMessage(false)
        {
        }

        virtual ~DiagResponseWrapper()
        {
            if (m_boDeleteMessage)
                DeleteDiagResponse(m_pDiagResp);
        }

        operator IDiagResponse&()
        {
            return *m_pDiagResp;
        }

        CAPL_METHOD_IMPL(CAPL_INT32, SendResponse)()
        {
            return m_pDiagResp->SendResponse();
        }

        CAPL_METHOD_IMPL(CAPL_INT32, IsNegativeResponse)() const
        {
            return m_pDiagResp->IsNegativeResponse();
        }

        CAPL_METHOD_IMPL(CAPL_INT32, IsPositiveResponse)() const
        {
            return m_pDiagResp->IsPositiveResponse();
        }

        CAPL_METHOD_IMPL(CAPL_INT32, GetResponseCode)() const
        {
            return m_pDiagResp->GetResponseCode();
        }

        IDiagResponse& operator=(IDiagResponse& m)
        {
            m_pDiagResp->CopyData(m);
            return *m_pDiagResp;
        }
        DiagResponseWrapper& operator=(DiagResponseWrapper& m)
        {
            m_pDiagResp->CopyData(*m.m_pDiagResp);
            return *this;
        }

        //ATTRIBUTE_WRAPPER_HOLDER(m_pDiagResp);
    };
    class DiagRequestWrapper : public DiagBaseWrapper
    {
        IDiagRequest* m_pDiagReq;
        /* Definiert ob m_pMsg durch die Erzeugung dieses Wrapper-Objektes
        * ebenfalls erzeugt wurde und somit auch von diesem wieder freigegeben
        * werden muss.
        */
        bool m_boDeleteMessage;
    public:
        explicit DiagRequestWrapper(const char* sHandle = "")
            : DiagBaseWrapper(NULL)
            , m_boDeleteMessage(true)
        {
            m_pDiagBase = m_pDiagReq = CreateDiagRequest(sHandle, GetStackName());
        }

        explicit DiagRequestWrapper(IDiagRequest& msg)
            : DiagBaseWrapper(&msg)
            , m_boDeleteMessage(false)
        {
        }

        virtual ~DiagRequestWrapper()
        {
            if (m_boDeleteMessage)
                DeleteDiagRequest(m_pDiagReq);
        }

        operator IDiagRequest&()
        {
            return *m_pDiagReq;
        }

        CAPL_METHOD_IMPL(CAPL_INT32, SendRequest)()
        {
            return m_pDiagReq->SendRequest();
        }

        IDiagRequest& operator=(IDiagRequest& m)
        {
            m_pDiagReq->CopyData(m);
            return *m_pDiagReq;
        }
        DiagRequestWrapper& operator=(DiagRequestWrapper& m)
        {
            m_pDiagReq->CopyData(*m.m_pDiagReq);
            return *this;
        }
    ...
    DiagEvent::DiagEvent()
    {
        m_hEvent = CreateDiagEvent(this);
    }

    DiagEvent::~DiagEvent()
    {
        DeleteDiagEvent(m_hEvent);
    }

    DiagRequestEvent::DiagRequestEvent(const char* sIds)
    {
        m_hEvent = CreateDiagRequestEvent(this, sIds, GetStackName());
    }

    DiagRequestEvent::~DiagRequestEvent()
    {
        DeleteDiagRequestEvent(m_hEvent);
    }

    DiagResponseEvent::DiagResponseEvent(const char* sIds)
    {
        m_hEvent = CreateDiagResponseEvent(this, sIds, GetStackName());
    }

    DiagResponseEvent::~DiagResponseEvent()
    {
        DeleteDiagResponseEvent(m_hEvent);
    }


    static CAPL_INT32 linSendWakeupLength(CAPL_INT32 ttobrk, CAPL_INT32 count, CAPL_UINT32 length)
    {
        return linSendWakeupInternal(GetCurBusContext(), ttobrk, count, length);
    }
    static CAPL_INT32 linSendWakeupCount(CAPL_INT32 ttobrk, CAPL_INT32 count)
    {
        return linSendWakeupLength(ttobrk, count, 250);
    }
    static CAPL_INT32 linSendWakeup()
    {
        return linSendWakeupCount(1, 1);
    }

    static CAPL_INT32 linChangeSchedTableOnInd(CAPL_UINT32 tableIndex, CAPL_UINT32 slotIndex, CAPL_UINT32 onSlotIndex)
    {
        return linChangeSchedTableInternal(GetCurBusContext(), tableIndex, slotIndex, onSlotIndex);
    }
    static CAPL_INT32 linChangeSchedTableInd(CAPL_UINT32 tableIndex, CAPL_UINT32 slotIndex)
    {
        return linChangeSchedTableOnInd(tableIndex, slotIndex, -2);
    }
    static CAPL_INT32 linChangeSchedTable(CAPL_UINT32 tableIndex)
    {
        return linChangeSchedTableInd(tableIndex, 0);
    }

    static CAPL_INT32 linSendSleepModFrm(CAPL_INT32 silent, CAPL_INT32 restartScheduler, CAPL_INT32 wakeupIdentifier)
    {
        return linSendSleepModFrmInternal(GetCurBusContext(), silent, restartScheduler, wakeupIdentifier);
    }

    static inline CAPL_INT32 getValueValueLongByObj(IEnvVar& var, CAPL_INT32& value)
    {
        return value = getValueLongByObj(var);
    }
    static inline CAPL_INT32 getValueValueLongByStringName(const char* sId, CAPL_INT32& value)
    {
        return value = getValueLongByStringName(sId);
    }
    static inline CAPL_INT32 DeferStop(DWORD maxDeferTime)
    {
        return DeferStopInternal(GetFileNameInternal(), maxDeferTime);
    }
    static inline void CompleteStop()
    {
        CompleteStopInternal(GetFileNameInternal());
    }
    ...
    //
    ////////////////////////////////////////////////////////
    static CAPL_INT32 TestWaitForMessageId(DWORD aMessageId, DWORD aTimeout)
    {
        return TestWaitForMessageIdInternal(GetCurBusContext(), aMessageId, aTimeout);
    }
    static CAPL_INT32 TestWaitForMessageAll(DWORD aTimeout)
    {
        return TestWaitForMessageAllInternal(GetCurBusContext(), aTimeout);
    }
    static CAPL_INT32 TestJoinMessageEventId(DWORD aMessageId)
    {
        return TestJoinMessageEventIdInternal(GetCurBusContext(), aMessageId);
    }
    ////////////////////////////////////////////////////////
    //
    //                     INI-File Access
    //
    ////////////////////////////////////////////////////////
    static char g_sWritePath[261];
    static char g_sReadPath[261];

    // setFilePath
    static inline void setFilePath(const char* cpPath, DWORD mode)
    {
        if (mode == 0 || mode == 2)
        {
            strcpy(g_sReadPath, cpPath);
            //g_sReadPath.Replace("\\\\", "\\"); // In CANoe muss man doppelte Backslashes angeben.
            DWORD len = strlen(g_sReadPath);
            if (len != 0 && g_sReadPath[len - 1] != '\\')
            {
                g_sReadPath[len] = '\\';
                g_sReadPath[len + 1] = 0;
            }
        ...
        if (mode == 1 || mode == 2)
        {
            strcpy(g_sWritePath, cpPath);
            //g_sWritePath.Replace("\\\\", "\\"); // In CANoe muss man doppelte Backslashes angeben.
            DWORD len = strlen(g_sWritePath);
            if (len != 0 && g_sWritePath[len - 1] != '\\')
            {
                g_sWritePath[len] = '\\';
                g_sWritePath[len + 1] = 0;
            }
        }
    }
    ...
    // setWritePath
    // 
    // This function sets the write path for the functions openFileWrite, writeProfileFloat, 
    // writeProfileInt and writeProfileString. The path can be given as absolute or relative 
    // to the currently active configuration.
    static inline void setWritePath(const char* cpPath)
    {
        // A file search procedure is used to determine the absolute file path. There are 
        // different procedures for writing and reading files. The search mechanism assume 
        // that every file open with the read only flag belongs to the CANoe/CANalyzer 
        // configuration. So the mechanism searches for the specified file in the directory 
        // of the first database. If the file could not be found there, the directories of 
        // further databases will be used. If the file still could not be found, the directory 
        // of the configuration file will be used.

        // For all files opend with the write flag the CAPL function setWritePath can be called 
        // to set a directory for all outstanding write operations. If setWritePath was not 
        // called before a write operation, the configuration directory will be used instead.

        strcpy(g_sWritePath, cpPath);
        //g_sWritePath.Replace("\\\\", "\\"); // In CANoe muss man doppelte Backslashes angeben.

        // Wenn noch kein abschließender Backslash vorhanden ist, dann einen dranhängen
        DWORD len = strlen(g_sWritePath);
        if (len != 0 && g_sWritePath[len - 1] != '\\')
        {
            g_sWritePath[len] = '\\';
    ...
    static inline CAPL_INT32 fileReadArray(const char* cpSection, const char* cpEntry, char* cpBuffer, short iBufferLen, const char* cpFilePath)
    {
        return fileReadArray_Internal(cpSection, cpEntry, cpBuffer, iBufferLen, cpFilePath, g_sReadPath, g_sWritePath);
    }

    // fileReadFloat (in CANoe als 'Obsolete' deklariert!)
    static inline double fileReadFloat(const char* cpSection, const char* cpEntry, double fDefault, const char* cpFilePath)
    {
        return fileReadFloat_Internal(cpSection, cpEntry, fDefault, cpFilePath, g_sReadPath, g_sWritePath);
    }


    // fileReadInt (in CANoe als 'Obsolete' deklariert!)
    static inline CAPL_INT32 fileReadInt(const char* cpSection, const char* cpEntry, CAPL_INT32 lDefault, const char* cpFilePath)
    {
        return fileReadInt_Internal(cpSection, cpEntry, lDefault, cpFilePath, g_sReadPath, g_sWritePath);
    }


    // fileReadString (in CANoe als 'Obsolete' deklariert!)
    static inline CAPL_INT32 fileReadString(const char* cpSection, const char* cpEntry, const char* cpDefault, char* cpBuffer, CAPL_INT32 lBufferLen, const char* cpFilePath)
    {
        return fileReadString_Internal(cpSection, cpEntry, cpDefault, cpBuffer, lBufferLen, cpFilePath, g_sReadPath, g_sWritePath);
    }


    // fileWriteFloat (in CANoe als 'Obsolete' deklariert!)
    static inline CAPL_INT32 fileWriteFloat(const char* cpSection, const char* cpEntry, double fValue, const char* cpFilePath)
    {
        return fileWriteFloat_Internal(cpSection, cpEntry, fValue, cpFilePath, g_sReadPath, g_sWritePath);
    }


    // fileWriteInt (in CANoe als 'Obsolete' deklariert!)
    static inline CAPL_INT32 fileWriteInt(const char* cpSection, const char* cpEntry, CAPL_INT32 lValue, const char* cpFilePath)
    {
        return fileWriteInt_Internal(cpSection, cpEntry, lValue, cpFilePath, g_sReadPath, g_sWritePath);
    }


    // fileWriteString (in CANoe als 'Obsolete' deklariert!)
    static inline CAPL_INT32 fileWriteString(const char* cpSection, const char* cpEntry, const char* cpValue, const char* cpFilePath)
    {
        return fileWriteString_Internal(cpSection, cpEntry, cpValue, cpFilePath, g_sReadPath, g_sWritePath);
    }

    // getProfileArray
    static inline CAPL_INT32 getProfileArray(const char* cpSection, const char* cpEntry, char* cpBuffer, CAPL_INT32 lBufferSize, const char* cpFilePath)
    {
        return getProfileArray_Internal(cpSection, cpEntry, cpBuffer, lBufferSize, cpFilePath, g_sReadPath, g_sWritePath);
    }


    // getProfileFloat
    static inline double getProfileFloat(const char* cpSection, const char* cpEntry, double fDefault, const char* cpFilePath)
    {
        return getProfileFloat_Internal(cpSection, cpEntry, fDefault, cpFilePath, g_sReadPath, g_sWritePath);
    }


    // getProfileInt
    static inline CAPL_INT32 getProfileInt(const char* cpSection, const char* cpEntry, CAPL_INT32 lDefault, const char* cpFilePath)
    {
        return getProfileInt_Internal(cpSection, cpEntry, lDefault, cpFilePath, g_sReadPath, g_sWritePath);
    }


    // getProfileString
    static inline CAPL_INT32 getProfileString(const char* cpSection, const char* cpEntry, const char* cpDefault, char* cpBuffer, CAPL_INT32 lBufferSize, const char* cpFilePath)
    {
        return getProfileString_Internal(cpSection, cpEntry, cpDefault, cpBuffer, lBufferSize, cpFilePath, g_sReadPath, g_sWritePath);
    }

    // writeProfileFloat
    static inline CAPL_INT32 writeProfileFloat(const char* cpSection, const char* cpEntry, double fValue, const char* cpFilePath)
    {
        return writeProfileFloat_Internal(cpSection, cpEntry, fValue, cpFilePath, g_sReadPath, g_sWritePath);
    }


    // writeProfileInt
    static inline CAPL_INT32 writeProfileInt(const char* cpSection, const char* cpEntry, CAPL_INT32 lValue, const char* cpFilePath)
    {
        return writeProfileInt_Internal(cpSection, cpEntry, lValue, cpFilePath, g_sReadPath, g_sWritePath);
    }


    // writeProfileString
    // 
    // Before this function can be called the write path must be set by the function setWritePath. 
    // Otherwise the configuration directory will be used. A relative file name must be passed to the function.
    static inline CAPL_INT32 writeProfileString(const char* cpSection, const char* cpEntry, const char* cpValue, const char* cpFilePath)
    {
        return writeProfileString_Internal(cpSection, cpEntry, cpValue, cpFilePath, g_sReadPath, g_sWritePath);
    }

    // openFileRead
    static inline DWORD openFileRead(const char* cpFilename, DWORD mode)
    {
        return openFileRead_Internal(cpFilename, mode, g_sReadPath, g_sWritePath);
    }


    // openFileWrite
    static inline DWORD openFileWrite(const char* cpFileName, DWORD dwMode)
    {
        return openFileWrite_Internal(cpFileName, dwMode, g_sReadPath, g_sWritePath);
    }
...
//####################################
//PluginCanFileSpecificApiVerbatimInclude DONE
//####################################
}; //namespace capl
namespace capl
{
   static char n_Helloworld[] = "Hello world!";
} //namespace capl

namespace capl
{
#undef __min
template<class T>static inline const T& __min(const T& l, const T& r){return l < r ? l : r;}
template<class T1, class T2>
void memcpy_h2n(T1& dest, const T2& src, size_t offset = 0)
{
   if (offset > sizeof(T1))
      return;
   const byte* last = (const byte*)&src + sizeof(T2) - 1;
   size_t count = __min(sizeof(T2), sizeof(T1) - offset);
   for (size_t i = 0; i < count; ++i)
   {
      ((BYTE*)&dest)[i + offset] = *(last - i);
   }
}
template<class T1, class T2>
void memcpy_n2h(T1& dest, const T2& src, size_t offset = 0)
{
   if (offset > sizeof(T2))
       return;
   size_t count = __min(sizeof(T1), sizeof(T2) - offset);
   for (size_t i = 0; i < count; ++i)
   {
      ((BYTE*)&dest)[sizeof(T1) - i - 1] = ((BYTE*)&src)[i + offset];
   }
}
#define GETSTRUCTMEMBERPOS(s, m) ((BYTE*)&s.m - (BYTE*)&s)
namespace n_main {
/* DEBUG::BEGIN */

  class C_0__1163FC50_456A_4143_BB43_FE78DA87055E
      : public StartEvent
  {
  public:
    C_0__1163FC50_456A_4143_BB43_FE78DA87055E()  /* DEBUG::SKIP */ { }
    virtual ~C_0__1163FC50_456A_4143_BB43_FE78DA87055E() {}  /* DEBUG::SKIP */
    virtual void CAPL_CALL OnEvent(IStart& THIS_HELPER)
    {
      StartWrapper This(THIS_HELPER);   /* DEBUG::SKIP */
      {
        capl::write(n_Helloworld);
      }
    }
  };
/* DEBUG::END */
...
void Init0()
{
    static n_main::C_0__1163FC50_456A_4143_BB43_FE78DA87055E Var_C_0__1163FC50_456A_4143_BB43_FE78DA87055E;
}
}; //namespace capl
