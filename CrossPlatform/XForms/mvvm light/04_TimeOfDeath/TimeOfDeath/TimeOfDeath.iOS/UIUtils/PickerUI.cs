...
namespace TimeOfDeath.iOS
{
   public class PickerUI
   {
      ...
      {
         var choiceModel = new PickerModel(param);
         var imp = param[0];
         ...
         toolHigh.SetItems(new[] {doneHigh}, true);

         var pickView = new UIView(new CGRect(0, 0, view.Bounds.Width, 260));
         pickView.AddSubviews(picker, toolHigh);

         choiceModel.PickerChanged += (sender, ea) => { imp = ea.SelectedValue; };
         return pickView;
      }
   }
}