...
namespace CustomBinderSample
{
   public sealed class SimpleBinder : Binder
   {
      ...
      private static readonly TypeCode[][] AllowedConversions = new TypeCode[19][];

      // This static constructor initializes the conversion table
      static SimpleBinder()
      {
         // For example, Char can be convert to SByte, Byte, or UInt16
         AllowedConversions[(int)TypeCode.Char] = new[]
         {TypeCode.SByte, TypeCode.Byte, TypeCode.UInt16};

         AllowedConversions[(int)TypeCode.Int16] = new[]
         {TypeCode.SByte, TypeCode.Byte};

         AllowedConversions[(int)TypeCode.UInt16] = new[]
         {TypeCode.Char, TypeCode.SByte, TypeCode.Byte};

         AllowedConversions[(int)TypeCode.Int32] = new[]
         {TypeCode.Char, TypeCode.SByte, TypeCode.Byte, TypeCode.Int16, TypeCode.UInt16};

         AllowedConversions[(int)TypeCode.UInt32] = new[]
         {TypeCode.Char, TypeCode.SByte, TypeCode.Byte, TypeCode.Int16, TypeCode.UInt16};

         AllowedConversions[(int)TypeCode.Int64] = new[]
         {
            TypeCode.Char, TypeCode.SByte, TypeCode.Byte, TypeCode.Int16, TypeCode.UInt16, TypeCode.Int32, TypeCode.UInt32
         };

         AllowedConversions[(int)TypeCode.UInt64] = new[]
         {
            TypeCode.Char, TypeCode.SByte, TypeCode.Byte, TypeCode.Int16, TypeCode.UInt16, TypeCode.Int32, TypeCode.UInt32
         };

         AllowedConversions[(int)TypeCode.Single] = new[]
         {TypeCode.Char, TypeCode.SByte, TypeCode.Byte, TypeCode.Int16, TypeCode.UInt16};

         AllowedConversions[(int)TypeCode.Double] = new[]
         {
      ...
      public override MethodBase BindToMethod(
         ...
         out object state)
      {
         // If 'args' is changed, this object can
      ...
      private static bool CompareParamAndArgTypes(
         ...
         CompareParamAndArgTypesFlags flags)
      {
         ...
         for (; i < paramTypes.Length; i++)
         {
            ...
            if (argTypes[i] != null)
            {
               var formalType = paramTypes[i].ParameterType;
      ...
      public override void ReorderArgumentArray(
         ...
         object state)
      {
         // This object can is used to get back
         // the original array
         ...
         ParameterModifier[] modifiers)
      {
         ...
         var numTests = ((BindingFlags.ExactBinding & bindingAttr) != 0) ? 1 : 3;

         for (var test = 0; test < numTests; test++)
         {
            var flags = CompareParamAndArgTypesFlags.Exact;
      ...
      private static ParameterInfo[] GetPropertyParams(PropertyInfo p)
      {
         ...
         m = p.GetSetMethod();
         var setParams = m.GetParameters();

         // Copy all elements but the last (the property's type) to a new array
         var paramTypes = new ParameterInfo[setParams.Length - 1];
      ...
      private static bool CanConvertPrimitiveType(Type fromType, Type toType)
      {
         ...
         if (AllowedConversions[(int)fromTypeCode] == null)
            return false;

         // Check table to see if conversion from fromType to toType is allowed
         var toTypeCode = Type.GetTypeCode(toType);
         if (Array.IndexOf(AllowedConversions[(int)toTypeCode], fromTypeCode) != -1)
            return true;
