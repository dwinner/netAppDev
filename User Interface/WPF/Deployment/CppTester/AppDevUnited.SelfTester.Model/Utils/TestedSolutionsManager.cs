...
namespace AppDevUnited.SelfTester.Model.Utils
{
   ...
   public static class TestedSolutionsManager
   {
      ...
      private static readonly Lazy<IEnumerable<string>> _DeferredLocalEtalons = new Lazy<IEnumerable<string>>(() =>
      {
      ...
      private static readonly Lazy<IEnumerable<string>> _DeferredLocalEtalonFileNames =
         new Lazy<IEnumerable<string>>(() => _DeferredLocalEtalons.Value.Select(Path.GetFileNameWithoutExtension));
      ...
      private static AnalysisStatus GetRemainingStatus(ICollection<string> pvsDiffs, string pvsLogFileName)
      {
         var status = pvsDiffs.Contains(pvsLogFileName)
            ...
            : (!_DeferredLocalEtalonFileNames.Value.Contains(pvsLogFileName)
