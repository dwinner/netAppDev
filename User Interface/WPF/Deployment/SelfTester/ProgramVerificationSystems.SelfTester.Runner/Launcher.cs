...
namespace ProgramVerificationSystems.SelfTester.Runner
{
   ...
   internal sealed class Launcher : IDisposable
   {
      ...
      private static readonly string[] ProcessesToKillOnStSessionEnd =
      ...
      private static void TakeScreenshot(IEnumerable<Process> processes)
      {
         foreach (var solutionName in from staleProc in processes
                                      select staleProc.GetCommandLine()
                                         into commandLine
                                         select SolutionRegex.Matches(commandLine)
                                            into slnMatch
      ...
      private void Dispose(bool disposing)
      {
         if (!_isDisposed)
         {
            if (disposing && _faultProcessMonitor != null)
            {
               ...
               ProcessesToKillOnStSessionEnd.SelectMany(Process.GetProcessesByName)
                  .ToList()
