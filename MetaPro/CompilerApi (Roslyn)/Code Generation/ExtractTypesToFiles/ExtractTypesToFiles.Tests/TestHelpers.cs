...
namespace ExtractTypesToFiles.Tests
{
   ...
   {
      internal static async Task TestProviderAsync(string file, string fileName,
         Func<Solution, ProjectId, Solution> modifySolution,
         Func<ImmutableArray<CodeAction>, Task> handleActions)
      {
         var code = File.ReadAllText(file);
         var document = CreateDocument(code, fileName, modifySolution);
         ...
         await handleActions(actions.ToImmutableArray()).ConfigureAwait(false);
      }
