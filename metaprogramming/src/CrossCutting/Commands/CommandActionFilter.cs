   ...
   public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
   {
      if (context.HttpContext.Request.Method == HttpMethod.Post.Method)
      {
         ...
         var commandResult = new CommandResult
         {
            ...
            ValidationResults = context.ModelState.SelectMany(
                  pair => pair.Value!.Errors.Select(modelError => modelError.ToValidationResult(pair.Key))),
         ...
         if (!commandResult.IsAuthorized)
         {
            context.HttpContext.Response.StatusCode =
               401; // Forbidden: https://www.rfc-editor.org/rfc/rfc9110.html#name-401-unauthorized
         }
         else if (!commandResult.IsValid)
         {
            context.HttpContext.Response.StatusCode =
               409; // Conflict: https://www.rfc-editor.org/rfc/rfc9110.html#name-409-conflict
         }
