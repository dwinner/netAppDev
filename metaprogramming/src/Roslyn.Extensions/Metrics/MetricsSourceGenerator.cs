...
public class MetricsSourceGenerator : ISourceGenerator
{
    ...
    {
        if (context.SyntaxReceiver is not MetricsSyntaxReceiver receiver)
        {
            return;
        }
        ...
        foreach (var candidate in receiver.Candidates)
        {
            ...
            var idValueText = candidate.Identifier.ValueText;
            var templateData = new MetricsTemplateData
            {
                Namespace = baseNsDeclSyntax!.Name.ToString(),
                ClassName = idValueText,
            };
            ...
            foreach (var member in candidate.Members)
            {
                ...
                if (methodSymbol is not null)
                {
                    ...
                    var attribute = attributes.FirstOrDefault(attrData =>
                        SymbolEqualityComparer.Default.Equals(attrData.AttributeClass?.OriginalDefinition, counterAttribute));
                    if (attribute is not null)
                    {
                        var tags = method.ParameterList.Parameters.Select(
                            parameter => new CounterTagTemplateData
                            {
            ...
            if (templateData.Counters.Count > 0)
            {
                ...
                context.AddSource($"{idValueText}.g.cs", source);
            }
        }
    }
