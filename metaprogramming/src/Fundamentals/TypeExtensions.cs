...
{
   private static readonly HashSet<Type> AdditionalPrimitiveTypes =
   [
      typeof(decimal),
      typeof(string),
      typeof(Guid),
   ...
   /// <summary>
   ///    Check if a type is enumerable. Note that string is an IEnumerable, but in this case the string is excluded, as well
   ///    as ExpandoObject.
   /// </summary>
   /// <param name="type"><see cref="Type" /> to check.</param>
   ...
   public static bool IsEnumerable(this Type type) =>
      ...
      && !type.IsString() && typeof(IEnumerable).IsAssignableFrom(type);
   ...
   public static bool IsComparable(this Type type) =>
      type.Implements(typeof(IComparable)) || type.ImplementsOpenGeneric(typeof(IComparable<>));
   ...
   public static bool Is<T>(this Type type) => type == typeof(T) || Nullable.GetUnderlyingType(type) == typeof(T);
   ...
   /// <param name="type"><see cref="Type" /> to check.</param>
   /// <returns>True if type is a date, false if not.</returns>
   public static bool IsDate(this Type type) => Is<DateTime>(type);
   ...
   public static bool IsDateOnly(this Type type) => Is<DateOnly>(type);
   ...
   public static bool IsTimeOnly(this Type type) => Is<TimeOnly>(type);
   ...
   /// <param name="type"><see cref="Type" /> to check.</param>
   /// <returns>True if type is a date, false if not.</returns>
   public static bool IsDateTimeOffset(this Type type) => Is<DateTimeOffset>(type);
   ...
   public static bool IsBoolean(this Type type) => Is<bool>(type);
   ...
   public static bool IsString(this Type type) => Is<string>(type);
   ...
   public static bool IsGuid(this Type type) => Is<Guid>(type);
   ...
   public static Type GetEnumerableElementType(this Type enumerableType)
   {
      ...
      return enumerableType.GetInterfaces()
         ...
         .Select(type => type.GenericTypeArguments[0]).FirstOrDefault()!;
   }
   ...
   public static bool HasInterface(this Type type, Type interfaceType)
   {
         ...
         return type.GetTypeInfo()
            ...
            .Count(lType =>
            {
               if (lType.IsGenericType 
                   && interfaceType.GetTypeInfo().GenericTypeParameters.Length == lType.GetGenericArguments().Length)
               {
                  var genericType = interfaceType.MakeGenericType(lType.GetGenericArguments());
                  return lType == genericType;
               }
      ...
      return type.GetTypeInfo()
         .ImplementedInterfaces
         .Count(lType => lType == interfaceType) == 1;
   }
   ...
   public static bool IsDerivedFromOpenGeneric(this Type type, Type openGenericType)
   {
      ...
      {
         var currentType = typeToCheck.GetTypeInfo().IsGenericType
   ...
   public static bool ImplementsOpenGeneric(this Type type, Type openGenericType)
   {
      ...
      return typeInfo.GetInterfaces()
         .Any(i => i.GetTypeInfo().IsGenericType &&
                   i.GetTypeInfo().GetGenericTypeDefinition().GetTypeInfo() == openGenericTypeInfo);
   }

   /// <summary>
   ///    Check if a type is a "primitve" type.  This is not just dot net primitives but basic types like string, decimal,
   ///    datetime,
   ...
   public static bool Implements(this Type type, Type super) => type.AllBaseAndImplementingTypes().Contains(super);
   ...
   public static bool IsRecord(this Type type) =>
      type.GetTypeInfo().DeclaredProperties.Any(x => x.Name == "EqualityContract");
