...
using MultiStudio.VsCodeInstallWizard.Utils;
        ...
        private async void OnDownload(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                ...
                if (vsCodeInslallerProc != null)
                {
                    ...
                    // If a user has choosen auto installation, then istall some extensions silently from the shell
                    if (exitCode == (short)ExitCodeKind.AutoInstalled)
                    {
                        const string vsCodeId = "Visual Studio Code";
                        var vsCodeEntity = InstalledPrograms.FindSoftwareByName(vsCodeId);
                        if (vsCodeEntity != null)
                        {
                            string vsCodePath = vsCodeEntity.DisplayIcon as string;
                            if (!File.Exists(vsCodePath))
                            {
                                throw new FileNotFoundException(
                                    $"File '{vsCodePath}' not found", vsCodePath);
                            }
                        ...
                        {
                            throw new FileNotFoundException(
                                $"Sowtware '{vsCodeId}' not found", vsCodeId);
                        }
                    }
