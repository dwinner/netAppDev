...
namespace AccountManagementSamples
{
   internal static class Program
   {
      ...
      private static void FindUsers1()
      {
         ...
         using (var users =
            UserPrincipal.FindByPasswordSetTime(context, DateTime.Today - TimeSpan.FromDays(30), MatchType.LessThan))
         {
            foreach (var user in users)
               Console.WriteLine("{0}, last logon: {1}, last password change: {2}", user.Name, user.LastLogon,
      ...
      private static void DisplayCurrentUser()
      {
         ...
         {
            Console.WriteLine("Context Name: {0}, Container: {1}, Server: {2}", user.Context.Name,
               user.Context.Container, user.Context.ConnectedServer);
      ...
      private static void FindUsers()
      {
         ...
         using (var searcher = new PrincipalSearcher())
         {
            ...
            foreach (var user in searchResult) Console.WriteLine(user.Name);
         }
      }
      ...
      private static void CreateUser()
      {
         using (var context = new PrincipalContext(ContextType.Domain, "explorer"))
         using (var user =
            new UserPrincipal(context, "Tom", "P@ssw0rd", true) {GivenName = "Tom", EmailAddress = "test@test.com"})
         {
            user.Save();
         }
      }
