...
namespace CSharp8Features
{
   public class Demo
   {
      ...
      public Demo()
      {
         ...
         WriteLine(months[^1]);  // the previous is the same

         var year = months[..];
         foreach (var s in year)
         {
            WriteLine(s); // January to December
         }

         var quarter = months[..3];
         foreach (var s in quarter)
         {
            WriteLine(s); // Quarter 1 - January to March
         }

         var restOfYear = months[3..];
         foreach (var s in restOfYear)
         {
            WriteLine(s); // April to December
         }
         ...
         // Error: var mystruct2 = new MyStruct<string> { One = "One", Two = "Two" };
      ...
      private void ReadFile()
      {
         ...
         using var reader = new StreamReader("C:\\temp\\TextDocument.txt");
         var lines = reader.ReadToEnd();
      }
   ...
   public struct DaysSince
   {
      ...
      public override readonly string ToString() => $"Days since {GivenDate} = {Number} days";
   }
   ...
   internal ref struct StudentScores
   {
      public void Dispose()
      {
         // perform clean up
