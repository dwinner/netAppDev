...
namespace LoggingSample
{
   internal class NetworkService
   {
      ...
      private static readonly ActivitySource ActivitySource = new("LoggingSample.DistributedTracing");
      ...
      public async Task NetworkRequestSampleAsync(Uri requestUri)
      {
         try
         {
            using var activity = ActivitySource.StartActivity("NetworkRequest");
            _logger.LogInformation(LoggingEvents.Networking, "NetworkRequestSampleAsync started with uri {uri}",
            ...
            _logger.LogInformation(LoggingEvents.Networking,
               "NetworkRequestSampleAsync completed, received {lenght} characters", result.Length);
         }
         catch (HttpRequestException ex)
         {
            _logger.LogError(LoggingEvents.Networking, ex,
               "Error in NetworkRequestSampleAsync, error message: {error}, HResult: {hresult}", ex.Message,
               ex.HResult);
         }
      }
   }
