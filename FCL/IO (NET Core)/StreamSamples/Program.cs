...
{
   Console.WriteLine(
      $"stream can read: {stream.CanRead}, can write: {stream.CanWrite}, can seek: {stream.CanSeek}, can timeout: {stream.CanTimeout}");
...
async Task RandomAccessSampleAsync()
{
   ...
   try
   {
      await using FileStream stream = File.OpenRead(sampleDataFilePath);
      ...
      do
      {
         try
         {
            ...
            if (int.TryParse(line, out var record))
            {
               ...
               string s = Encoding.UTF8.GetString(buffer.Span[..read]);
