...
CommandLineBuilder BuildCommandLine()
{
   ...
   deleteDuplicateFilesCommand.Handler =
      CommandHandler.Create<string, bool?>((dir, checkOnly) => DeleteDuplicateFiles(dir, checkOnly ?? true));
...
void ShowSpecialFolders()
{
   ...
   {
      var folder = Enum.Parse<Environment.SpecialFolder>(specialFolder);
...
void DeleteDuplicateFiles(string directory, bool checkOnly = true)
{
   ...
   foreach (string fileName in fileNames)
   {
      ...
      var ix = previousFileName.LastIndexOf(" - Copy", StringComparison.Ordinal);
