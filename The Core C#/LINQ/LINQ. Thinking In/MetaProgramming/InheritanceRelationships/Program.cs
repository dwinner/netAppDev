            ...
            Assemblies.SelectMany(
               dir =>
                  Assembly.LoadFrom(dir)
                     ...
                     .Select(t => new { Parent = t.BaseType, t.Name }))
               .Where(arg => arg.Parent != null)
               .Select(arg => new { Parent = arg.Parent.Name, arg.Name })
               .ToLookup(arg => arg.Parent);
         inheritanceRel.ForEach(group => Console.WriteLine("{0}: {1}", @group.Key, @group.Count()));
      }
   }
}