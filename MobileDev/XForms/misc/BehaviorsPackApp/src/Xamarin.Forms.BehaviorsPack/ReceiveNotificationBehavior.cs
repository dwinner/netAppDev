...
namespace Xamarin.Forms.BehaviorsPack
{
   ...
   {
      private static readonly MethodInfo HandlerMethodInfo =
         typeof(ReceiveNotificationBehavior<TBindableObject, TEventArgs>).GetRuntimeMethods()
            .Single(mi => mi.Name == "OnReceived" && mi.IsPublic == false);

      public static readonly BindableProperty EventNameProperty =
         BindableProperty.Create(nameof(EventName), typeof(string),
            typeof(ReceiveNotificationBehavior<TBindableObject, TEventArgs>), propertyChanged: OnEventNameChanged);

      public static readonly BindableProperty NotificationRequestProperty =
         BindableProperty.Create(nameof(NotificationRequest), typeof(INotificationRequest<TEventArgs>),
            typeof(ReceiveNotificationBehavior<TBindableObject, TEventArgs>),
            propertyChanged: OnInterractionRequestChanged);
