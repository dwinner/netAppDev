...
using Camera.iOS.Renderers.CustomImage;
...
namespace Camera.iOS.Renderers.CustomImage
{
   ...
   public class CustomImageRenderer : ViewRenderer<Controls.CustomImage, UIView>
   {
      #region Constructors
      ...
      protected override void OnElementChanged(ElementChangedEventArgs<Controls.CustomImage> e)
      {
      ...
      ///    The system version.
      /// </summary>
      private readonly int _systemVersion = Convert.ToInt16(UIDevice.CurrentDevice.SystemVersion.Split('.')[0]);
      ...
      private UIImage ReadBitmapImageFromStorage(string fn)
      {
         ...
         var filePath = Path.Combine(Environment.CurrentDirectory, fn);
      ...
      private void UpdateControlColor()
      {
         ...
         {
            var color = UIColor.Clear.FromHex(Element.TintColorString);
