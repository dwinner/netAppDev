    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (args.Length > 0)
            {
                ...
                dataTarget = DataTarget.LoadCrashDump(args[0]);
            }
            else
            {
                ...
                Thread.Sleep(1000);
                dataTarget = DataTarget.AttachToProcess(
                    targetProcess.Id, 
                    10000, // timeout
                    AttachFlag.Invasive);
            }
            ...
            { 
                ...
                string dacFile = GetDacFile(dataTarget.ClrVersions[0], dataTarget);
        ...
        private static void PrintFinalizableObjects(ClrRuntime clr)
        {
            ...
            foreach (var objAddr in clr.Heap.EnumerateFinalizableObjectAddresses())
            {
        ...
        private static void PrintRootsOfObjects(ClrRuntime clr)
        {
            ...
            Dictionary<ulong, ClrObject> childToParents = new Dictionary<ulong, ClrObject>();
            var heap = clr.Heap;
        ...
        private static void PrintTop10BiggestMethods(ClrRuntime clr)
        {
            ...
            for (int i=0;i<Math.Min(10, methods.Count);i++)
            {
                ...
                Console.WriteLine($"{method.Module}, {method.TypeName}, {method.Name}, {method.ILSize}, {method.NativeSize}");
            }
        }   
        ...
        private static void PrintHeapInfo(ClrRuntime clr)
        {
            ...
            Console.WriteLine($"Heap Count: {clr.HeapCount}");
            var heap = clr.Heap;
            Console.WriteLine($"Size: {heap.TotalHeapSize:N0} bytes");
            Console.WriteLine($"Segment Count: {heap.Segments.Count}");                
