...
let cases =
   FSharpType.GetUnionCases(typeof<Expression>)
   |> Array.map (fun f -> (f.Name, FSharpValue.PreComputeUnionConstructor(f)))
   |> Map.ofArray
...
let rec recursiveBuild (root: XElement) =
   ...
   let makeCase parameters =
      ...
      with exp ->
         raise
         <| Exception(String.Format("Failed to create {0} : {1}", name, exp.Message))

   let elems = root.Elements() |> Seq.toArray
   let values = elems |> Array.map recursiveBuild
...
let main _ =
   ...
   printf $"%s{print parsed} = %d{eval parsed}"
