...
internal static class Program
{
   ...
   private static void ReplaySubjectExample()
   {
      ...
      var sbj = new ReplaySubject<int>(3, TimeSpan.FromSeconds(1));
   ...
   {
      Demo.DisplayHeader(
         "ReplaySubject - simulating the usage of RxToBand for showin the heart rate - https://github.com/Reactive-Extensions/RxToBand");

      var heartRate = Observable.Range(70, 5);
      var sbj = new ReplaySubject<int>(20, TimeSpan.FromMinutes(2));
...
// after a while (for example, after the user selected to show the heart rate on the screen)
   ...
   private static void MergingBySubjectPitfall()
   {
      ...
      messagesFromDb.ToObservable()
         .Subscribe(sbj); //we can omit the ToObservable() since there is builtin "Subscribe" extension method 
   ...
   {
      Demo.DisplayHeader(
         "Subject - even with many source, publish the notification it observe, but stops after OnCompleted was called");
