...
internal static class TypesOfSchedulersExamples
{
   ...
   {
      Demo.DisplayHeader(
         "ImmediateScheduler - Uses the current thread (the caller thread) for each scheduling but runs the scheduled action immediatly");
   ...
   {
      Demo.DisplayHeader(
         "ImmediateScheduler - Future dueTime will cause the ImmediateScheduler block until the dueTime");

      var immediateScheduler = ImmediateScheduler.Instance;

      var countdownEvent = new CountdownEvent(2);
      Console.WriteLine("Calling thread: {0} Current time: {1}", Thread.CurrentThread.ManagedThreadId,
   ...
   {
      Demo.DisplayHeader(
         "EventLoopScheduler - Recursive scheduling will be enqueued. When an action finish, the next one is dequeued");
   ...
   {
      Demo.DisplayHeader(
         "SynchronizationContextScheduler - A bridge between Rx schedulers world and .NET SynchronizationContext");
