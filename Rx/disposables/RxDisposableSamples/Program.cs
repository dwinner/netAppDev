...
internal static class Program
{
   ...
   private static void TestMySubscribeOn()
   {
      ...
      Observable.Return(10)
         ...
         .Do(_ => Console.WriteLine($"Thread Id {Thread.CurrentThread.ManagedThreadId}"))
         .Wait();
   }
   ...
   {
      Demo.DisplayHeader(
         "The RefCountDisposable - dispose the underlying disposable when all referencing disposables are dispsosed");
   ...
   {
      Demo.DisplayHeader(
         "AddToCompositeDisposable extensions method - useful for keeping your observable pipeline fluent");

      var compositeDisposable = new CompositeDisposable();
      var observable = Ex.FromValues("Rx", "For", "The", "Win");
