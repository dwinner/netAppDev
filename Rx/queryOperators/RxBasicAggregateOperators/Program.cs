...
namespace RxBasicAggregateOperators;
...
internal static class Program
{
   ...
   {
      Demo.DisplayHeader(
         "The Aggregate operators - the resultSelector function transforms the final accumulator value into the result value");
      ...
      numbers.Aggregate(
            ...
            largest => largest
               .FirstOrDefault()) //since the collection is sorted and contain two items at most, the first items is the second largest one 
         .SubscribeConsole();
   ...
   {
      Demo.DisplayHeader(
         "The Aggregate and Scan operators - apply a function to each item emitted by an Observable, sequentially, and emit the final value (aggregate) or each computed value (scan) ");
   ...
   {
      Demo.DisplayHeader(
         "The MaxBy and MinBy operators - the provided selector will determine the maximal/minimal object based on the selected value");

      var grades = new Subject<StudentGrade>();
      grades.MaxBy(studentGrade => studentGrade.Grade)
         .SelectMany(max => max)
   ...
   {
      Demo.DisplayHeader(
         "The Max and Min operators - passing a selector will emit the maximal/minimal from the values produced by the selector");

      var grades = new Subject<StudentGrade>();
      grades.Max(studentGrade => studentGrade.Grade)
   ...
   {
      Demo.DisplayHeader(
         "The Count operator - you can also specify a predicate that will filter some of values from being counted");
