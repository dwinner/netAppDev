...
using RxBasicQueryOperators.Examples;
using RxBasicQueryOperators.Model;
using RxHelpers;

namespace RxBasicQueryOperators;
...
internal static class Program
{
   ...
   {
      Demo.DisplayHeader(
         "The Where operator - filters the elements of an observable sequence based on a predicate ");
   ...
   {
      Demo.DisplayHeader(
         "The DistinctUntilChanged operator - returns an observable sequence that contains only distinct contiguous element");
   ...
   {
      Demo.DisplayHeader(
         "The Distinct operator - filters values that were already emmited by the observable - by a given keySelector");

      var subject = new Subject<NewsItem>();
      subject.Log()
         .Distinct(newsItem => newsItem.Title) //items with same Title will only be emitted once
         .SubscribeConsole("Distinct");
      ...
      var messagesViewModels =
         messages
            .Select(message => new ChatMessageViewModel
            {
               ...
               User = LoadUserFromDb(message.Sender)
            }).Log();
   ...
   private static void Selecting()
   {
      ...
      Observable.Interval(TimeSpan.FromMilliseconds(500)).Take(5)
         ...
         .Select(message => new
         {
            ...
            LocalTime = message.Timestamp.ToLocalTime()
         })
         .Log()
         .Wait();
   }
