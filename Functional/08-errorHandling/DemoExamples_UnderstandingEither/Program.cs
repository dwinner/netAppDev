...
static Either<string, int> Test1(int a, int b)
{
   if (a < 0)
   {
      return $"Error: the first argument {a} is negative";
   }

   if (b == 0)
   {
      return $"Error: The second argument is {b}";
   }

   if (b > 100)
   {
      return $"Error: {b} exceeds 100";
   }
...
static void Test2(Either<string, int> val)
{
   val.Match
   (
      x => WriteLine($"Correct value: {x}"),
      x => WriteLine($"Incorrect value: '{x}'")
   );
}