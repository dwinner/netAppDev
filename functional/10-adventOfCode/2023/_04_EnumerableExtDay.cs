...
public class _04_EnumerableExtDay
{
...
Card 6: 31 18 13 56 72 | 74 77 10 23 35 67 36 11".Split(Environment.NewLine)
      ...
      var scoringCards = progress.IterateUntil(x => x.cards.Length - 1 == x.Index,
         agg =>
         {
            ...
            var id = int.Parse(card.Substring(5, card.IndexOf(":", StringComparison.Ordinal) - 5));
            var duplicateCards = cards.Skip(id).Take(score);
            var newCards = agg.cards
               .Concat(duplicateCards)
               .OrderBy(x => x.Card)
               .Select((x, i) => (x.Card, i))
               .ToArray();
            return (
...
Card 6: 31 18 13 56 72 | 74 77 10 23 35 67 36 11".Split(Environment.NewLine)
      ...
      var finalTotal = cards.Aggregate(cardCounts, (agg, x) =>
      {
         ...
         var updatedCounts =
            agg.Select(y => (y.Id, y.Id.IsBetween(x.Key + 1, x.Key + score) ? y.Count + count : y.Count));
