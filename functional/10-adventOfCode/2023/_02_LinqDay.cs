...
public class _02_LinqDay
{
   private static GameTurn ParseGameTurn(string input)
   {
      ...
      {
         "red"
            when x.Qty > agg.MaxRed => agg with { MaxRed = x.Qty },
         "blue"
            when x.Qty > agg.MaxBlue => agg with { MaxBlue = x.Qty },
         "green"
            when x.Qty > agg.MaxGreen => agg with { MaxGreen = x.Qty },
         _ => agg
      });
   ...
   private static int CalculatePossibleTurnTotal(IEnumerable<GameTurn> turns,
      int red = 12,
      int green = 13,
      int blue = 14)
   {
