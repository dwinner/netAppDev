...
public record Grid<T>
{
   public Grid(string rawInput, string lineSplit, string cellSplit, Func<int, int, string, T> valueFactor)
   {
      var g = rawInput.Split(lineSplit)
         .SelectMany((l, y) =>
            l.Split(cellSplit).Select((r, x) => new GridPoint<T> { Value = valueFactor(x, y, r), X = x, Y = y }))
         .ToArray();
   ...
   {
      var g = rawInput.Split(lineSplit)
         .SelectMany((l, y) => l.ToArray().Select(x => x.ToString())
            .Select((r, x) => new GridPoint<T> { Value = valueFactor(x, y, r), X = x, Y = y })).ToArray();
   ...
   public Grid(int sizeX, int sizeY, Func<int, int, T> valueFactory)
   {
      ...
      Points = Enumerable.Range(0, sizeX)
