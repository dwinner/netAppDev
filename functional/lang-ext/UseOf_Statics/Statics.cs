...
using System.Linq;
using LanguageExt;
using UseOf_Statics.Failures;

namespace UseOf_Statics;
...
public static class Statics
{
   ...
   public static Unit Success => new();
   ...
   /// <param name="eithers"></param>
   /// <returns></returns>
   public static Either<IAmFailure, IEnumerable<Either<IAmFailure, T>>> AggregateFailures<T>(
      this IEnumerable<Either<IAmFailure, T>> eithers)
   {
   ...
   public static Either<IAmFailure, T> ToEither<T>(this Option<T> value)
      => value.Match(t => Prelude.Right(t), () => ShortCircuitFailure.Create("None").ToEitherFailure<T>());

   public static Either<IAmFailure, T> ToEither<T>(this Option<T> value, string FailureReason)
      => value.Match(t => Prelude.Right(t), () => ShortCircuitFailure.Create(FailureReason).ToEitherFailure<T>());

   /// <summary>
   ///    Make an IFailure to a Either&lt;IFailure, T&gt; ie convert a failure to a either that represents that failure ie an
   ///    either in the left state
   /// </summary>
   /// <typeparam name="T"></typeparam>
   ...
   public static Option<T> ToSome<T>(this T t)
      => t == null ? Prelude.None : Prelude.Some(t);
      ...
      => new Try<T>(() => action())
         .Match(
            unit => unit == null
               ? failure.ToEitherFailure<L, T>()
               : unit.ToEither<L, T>(),
            exception => failure.WithException(exception));
   ...
   public static TRight ThrowIfFailed<TLeft, TRight>(this Either<TLeft, TRight> either, IAmFailure specificFailure)
      where TLeft : IAmFailure
   ...
   public static Either<IAmFailure, Unit> IgnoreFailure<L, R>(this Either<L, R> either) where L : IAmFailure
      => either.Match(r => Nothing.ToEither(), left => Nothing.ToEither());

   public static Either<IFailure, Unit> IgnoreFailureOf<IFailure, R>(this Either<IFailure, R> either, Type failureType)
      => either.Match(right => Nothing.ToEither<IFailure, Unit>(),
         failure => WhenTrue(() => failure.GetType() == failureType).ToEither()
            .Match(found => found.ToEither<IFailure, Unit>(),
               notFound => failure.ToEitherFailure<IFailure, Unit>()));

   public static Either<IFailure, R> IgnoreFailureOfAs<IFailure, R>(this Either<IFailure, R> either, Type failureType,
      R OnFail)
      => either.BindLeft(failure => WhenTrue(() => failure.GetType() == failureType)
         .Match(unit => OnFail.ToEither<IFailure, R>(),
            () => failure.ToEitherFailure<IFailure, R>()));

   public static Option<Unit> IgnoreNone<T>(this Option<T> option)
      => option.Match(some => Prelude.Some(some), () => Prelude.Some(Nothing));
   ...
   public static Either<IAmFailure, T> EnsuringBind<R, T>(this Either<IAmFailure, R> either,
      Func<R, Either<IAmFailure, T>> transformingFunction)
      => either
         .Bind(right => _ensuringBindReturn(() => transformingFunction(right),
