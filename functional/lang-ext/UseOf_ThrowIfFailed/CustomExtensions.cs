...
// Note a 'Successful' either is in the Right state while an 'unsucessful' either is in Left state
public static class EitherExtensions
{
   ...
   public static Either<IAmFailure, Option<T>> FailureToNone<T>(this Either<IAmFailure, Option<T>> item,
      Func<IAmFailure, bool> predicate)
   ...
   public static Either<IAmFailure, T> NoneToFailure<T>(this Either<IAmFailure, Option<T>> item,
      ...
      => item.Bind(e => e.Match(value => value.ToSuccess(), () => Prelude.Left<IAmFailure, T>(failure())));

   /// <summary>
   ///    Given An Either with an optional Right value. If option value is valid/some, make a Either Failure using provided
   ///    failure, otherwise a Successful Either
   /// </summary>
   public static Either<IAmFailure, Option<T>> SomeToFailure<T>(this Either<IAmFailure, Option<T>> item,
      Func<IAmFailure> failure)
      => item.Bind(either =>
         either.Match(_ => Prelude.Left<IAmFailure, Option<T>>(failure()), () => either.ToSuccess()));

   public static Func<IEnumerable<Option<T>>, Either<IAmFailure, IEnumerable<T>>> NoneToFailure<T>(
      Func<IAmFailure> failure)
      => items => items.Sequence()
         .Match(item => item.ToSuccess(), () => Prelude.Left<IAmFailure, IEnumerable<T>>(failure()));

   /// <summary>
   ///    Make an Either
   ///    'IAmFailure, T'
   ///    in left (failure) state if the list is empty (use failure producing function provided), otherwise
   ///    return either in right state using the first item in the list as the light value in the either
   /// </summary>
